...
async function handleEbayApi(request, env) {
  ...
  try {
    ...
    const entries = await Promise.all(
      Object.entries(SECTION_QUERIES).map(async ([key, sectionQuery]) => {
        const items = await searchItems(accessToken, sectionQuery);
        return [key, items];
      })
    );
...
export default {
  fetch(request, env) {
    ...
    if (url.pathname === '/api/ebay') {
      return handleEbayApi(request, env);
    }
